...
async function runTest() {
  ...
  try {
    ...
    const stageNoCells = await driver.findElements(By.xpath('//table//tr/td[1]'));
    ...
    await driver.wait(until.titleContains('JobStageApp'), 5000);
    ...
    await jobNoSelect.sendKeys(testJobNo, Key.RETURN);
    ...
    const stageDescInput = await driver.findElement(By.css('#stageDescInput'));
    ...
    const startDateInput = await driver.findElement(By.css('#startDateInput'));
    await startDateInput.clear();
    await startDateInput.sendKeys('2025-06-01');

    console.log('Filling End Date...');
    const endDateInput = await driver.findElement(By.css('#endDateInput'));
    ...
    await qapSelect.sendKeys(testQAP, Key.RETURN);
    ...
    const stageRows = await driver.findElements(By.xpath(`//td[text()='${newStageNo}']`));
    if (stageRows.length > 0) {
      ...
      const deleteButton = await driver.findElement(
        By.xpath(`//td[text()='${newStageNo}']/following-sibling::td//button[contains(text(), 'Delete')]`)
      );
      ...
      // Confirm deletion
      const stageRowsAfterDelete = await driver.findElements(By.xpath(`//td[text()='${newStageNo}']`));
